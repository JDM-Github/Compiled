...
function login()
{
    ...
    $stmt = $database->prepare("SELECT id, userName, birthDate, address, password FROM users WHERE email = ?");
    ...
    if ($stmt->num_rows === 1) {
        $stmt->bind_result($userId, $userName, $birthDate, $address, $hashedPassword);
        $stmt->fetch();
        ...
        if (password_verify($password, $hashedPassword)) {
            ...
            $stmt = $database->prepare("SELECT COUNT(*) FROM resume WHERE user_id = ?");
            $stmt->bind_param("i", $userId);
            $stmt->execute();
            $stmt->bind_result($resumeCount);
            $stmt->fetch();
            $stmt->close();
            ...
            if ($resumeCount > 0) {
                ...
                $_SESSION['user']['haveResume'] = false;
            }
...
function register()
{
    ...
    $stmt = $database->prepare("INSERT INTO users (userName, firstName, lastName, email, password, birthDate, address) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param('sssssss', $userName, $firstName, $lastName, $email, $hashedPassword, $birthDate, $address);
